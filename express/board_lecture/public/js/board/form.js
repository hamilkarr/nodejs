$(function () {
  ...
  /** 이미지 본문 추가 */
  //$(".uploaded_files .insert_editor").click(function () {
  $("body").on("click", ".uploaded_files .insert_editor", function () {
    const url = $(this).closest("li").data("li");
...
function callbackFileUpload(data) {
  if (!data) return;
  let fileInfoHtml = `<li data-idx=${data.idx} data-url='${data.uploadedURL}'><a href='/file/download/${data.idx}'>${data.origianlName}</a>`;
  if (data.uploadType == "board_editor") {
    // 에디터 첨부 이미지
    fileInfoHtml += `<i class="xi-file-upload insert_editor"></i>`;
    CKEDITOR.instances.content.insertHtml(`<img src='${data.uploadedURL}'>`);
  }
  fileInfoHtml += `<i class='xi-close-min delete'></i></li>`;
