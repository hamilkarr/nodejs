...
const board = {
  ...
  async getList(boardId, req, limit) {
    ...
    try {
      if (!boardId) throw new Error("게시판 아이디 누락..");
      ...
      /** 게시글 전체 개수 */
      const countSql = commonSql
        .replace("%fields%", "COUNT(*) cnt")
      ...
      const sql = commonSql
        .replace("%fields%", "a.*, b.memId, b.memNm")
        .replace("%addQuery%", addQuery);
...
<<<<<<< Updated upstream
  ...
  updateViewCount(idx, req) {
    ...
    try {
      ...
      // 이미 본 게시글인 경우 browserId + idx 추가시 오류 발생
    }

    /** 현재 게시글 조회수 총 합 -> 게시글에 업데이트 */
    try {
