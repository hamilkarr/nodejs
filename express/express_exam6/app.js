...
const upload = multer({
    ...
    storage: multer.diskStorage({ // 현재 서버 로컬 폴더에 저장 설정 
        destination: (req, file, done) => { // 업로드될 디렉토리 경로
        ...
        filename: (req, file, done) => {
           ...
           path
               .basename : 파일 이름
                       -> 두번째 인수로 파일의 확장자를 명시 -> 확장자 없는 파일 이름.
...
app.use(session({
    resave: false,
    // 같은 값을 저장할때 다시 저장할 것인가? false : 저장 안하고 기존꺼 유지
...
/** 라우터 등록 */
// app.use(indexRouter);
app.use("/file", fileRouter); // /file/filerou
