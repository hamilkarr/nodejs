    ...
    */
    join : async function(data) {
        try {
            ...
            const filePath = path.join(_dirname,"../data/member", data.memId + ".json");
    ...
    /* 로그인처리
        req인수가 추가된 이유 -> 로그인 아이디와 비번이 일치하는 경우 세션에서 회원 아이디를 저장하기 위한 용도 */
    ...
    login : function (memId, memPw, req) {
        ...
        4. 공통 미들웨어 -> 초기화 담당 미들웨어를 추가해서 로그인 데이터 전역 유지
            (공통 미들웨어는 세션 미들웨어가 등록된 이후에 추가해야 정상적으로 세션 데이터를 확인할 수 있다.) */
